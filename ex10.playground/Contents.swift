...
//참조 타입 : 변수 안에 실ㅈㅔ 값이 아닌, 그 값을 가진 메모리 주소값을 정수로 가진다.
...
//스위프느는 값 타입을 선호함. 값 전달이 편해서. 구조체를 선호함.

//구조체 : Struct 변수와 함수를 사용 가능. 값 타입 , 복사가 간편. 상속이 불가능
//열거형 : Enum 변수와 함수를 사용 가능. 값 타입 , 복사가 간편. 상속이 불가능
//클래스 : Class 변수와 함수를 사용 가능. 참조 타입 , 복사가 복잡. 상속이 가능함
...
//구조체의 객체 생성
let myStruct = MyStruct() //new 키워드 안씀. 사라짐.
var myStruct2 = myStruct; //age의 실제값이 복사됨.
myStruct2.age = 33
print(myStruct.age)
print(myStruct2.age)
//힌트 : myStruct 와 myStruct2는 서로 다른 메모리 공간에 할당됨.
...
print(myClass.age)
print(myClass2.age)
//힌트 : myClass 와 myClass2는 주소값이 동일함. 같은 메모리공간을 가리키고 있음.
